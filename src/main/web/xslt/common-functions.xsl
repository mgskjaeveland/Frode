...
<!-- Copyright (c) 2010-2013, Silvio Peroni <essepuntato@gmail.com> Permission 
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:function name="f:string-last-index-of" as="xs:integer*">
		...
		<xsl:variable name="result" select="f:string-index-of($source,$pattern)"
			as="xs:integer*" />
	...
	<xsl:function name="f:string-index-of" as="xs:integer*">
		...
		<xsl:variable name="tokens" select="tokenize($source,$pattern)"
			as="xs:string+" />
		<xsl:sequence
			select="for $i in (1 to count($tokens) - 1) return sum(for $token in $tokens[position() &gt;= 1 and position() &lt;= $i] return string-length($token)) + $i" />
	</xsl:function>
		...
		<function>
			...
			<xsl:call-template name="get.test">
				...
				<xsl:with-param name="result"
					select="f:string-first-index-of($p1,$p2)" />
			</xsl:call-template>
	...
	<xsl:template match="xsl:function[@name = 'f:string-last-index-of']">
		...
		<function>
			...
			<xsl:call-template name="get.test">
				<xsl:with-param name="params" select="($p1,$p2)" />
				<xsl:with-param name="result"
					select="f:string-last-index-of($p1,$p2)" />
			</xsl:call-template>
	...
	<!-- <xsl:template match="text()[ancestor::element()[namespace-uri() = 'http://www.w3.org/1999/XSL/Transform']]" 
		/> <xsl:template match="/"> <tests> <xsl:apply-templates /> </tests> </xsl:template> -->
